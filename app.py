...
app.layout = html.Div([
    ...
    dcc.Markdown("""
                 ...
                 Researches can be found on the Internet that nutritional values (such as sodium, fat, and sugar) of certain McDonald’s menu are way beyond the amounts we need per meal/day. Over the years, McDonald’s have been adding healthier items to their menus like salads and trying to make the existing items less greasy and salty. But for most people, going to McDonald’s is still for items like Big Mac or other signature meals and sandwiches. 
                    Therefore, this dashboard is created for all McDonald’s lover to see the nutritional values of their favorite menus. This dashboard contains some of the most popular items from McDonald’s breakfast, burger, and dessert menu. A user will be able to click on the item(s) they are interested in from the dropdown and see the calories of the items, which will display in a histogram below. The dropdown is predetermined to select all the items, but users can still select/de-select different items to compare calories among them. 
                    In the bottom of the dashboard, users may also choose one of the three important nutritional values (total sugars, total fat, and protein) from the dropdown. Once the user chooses a value, a pie chart will display the amounts of the value containing in different items we have. When the user places
                 their cursor on a part of the pie chart, the user will be able to see the item and the the value of user's choice. This way, users may compare nutritional values among various items and choose a healthier option from there.   
                 """),
    html.Div([
    dcc.Dropdown(
        id='values', 
